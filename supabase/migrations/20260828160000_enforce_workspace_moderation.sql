-- Suspension must be an enforcement boundary, not only a dashboard label.
...
begin
  ...
  if not nexora_private.can_manage_workspace(p_workspace_id) then
    raise exception using errcode = 'P0001', message = 'not_authorized';
  end if;

  select w.operational_status into workspace_status
  ...
  if char_length(raw_key) <> 25 then
    raise exception using errcode = 'P0001', message = 'key_generation_failed';
  end if;
