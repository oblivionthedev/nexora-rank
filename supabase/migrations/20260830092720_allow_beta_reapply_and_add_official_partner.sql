...
begin
  ...
  select * into discord_link from public.account_links where user_id = actor and provider = 'discord';
  ...
  where lower(email) = clean_email or discord_user_id = discord_link.provider_user_id
  ...
  if existing.id is not null then
    ...
    return jsonb_build_object('ok', true, 'application_id', application_id, 'lookup_code', raw_code, 'status', 'submitted', 'reapplied', true);
  end if;
...
do $$
declare platform_owner uuid;
begin
  select user_id into platform_owner
  from public.staff_members
  where active = true and role = 'owner'
  order by created_at
  limit 1;
  if platform_owner is not null then
    insert into public.partners (
      ...
      roblox_owner_display_name, discord_invite_url, published, created_by
    ) values (
      '596263047', 'Nexora Rank',
      'https://tr.rbxcdn.com/180DAY-c29c78f68d8d924eeb0c13198c115a63/150/150/Image/Png/noFilter',
      1, '5490600888', 'OblivionTheDev', 'OblivionDev',
      'https://discord.gg/YY9nXqqWTk', true, platform_owner
    )
