...
begin
  if actor is null then
    return jsonb_build_object('ok', false, 'error', 'discord_required');
  end if;
  ...
  if discord_link.provider_user_id is null then
    return jsonb_build_object('ok', false, 'error', 'discord_required');
  end if;
  ...
  where lower(application.email) = clean_email
     or application.discord_user_id = discord_link.provider_user_id
  ...
  insert into nexora_private.beta_applications (
    ...
    clean_name, clean_email, applicant_age,
  ...
  return jsonb_build_object('ok', true, 'application_id', application_id,
    'lookup_code', raw_code, 'status', 'submitted');
end
$$;

revoke all on function nexora_private.submit_beta_application(text, text, integer)
  from public, anon;
grant execute on function nexora_private.submit_beta_application(text, text, integer)
  to authenticated;

revoke all on function public.submit_beta_application(text, text, integer)
  from public, anon;
grant execute on function public.submit_beta_application(text, text, integer)
  to authenticated;

comment on function nexora_private.submit_beta_application(text, text, integer) is
  'Accepts Beta applications only from authenticated users with a verified Discord identity; live server membership is additionally checked by the server action.';
