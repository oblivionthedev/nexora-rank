...
import {
  Activity, ArrowRight, Blocks, Bot, Check, Copy, FileCheck2, Fingerprint,
          ...
          <div className="mt-3.5 grid gap-3.5 lg:grid-cols-[1.1fr_.9fr]">
            <section className="glass p-6 sm:p-7">
              ...
              <div className="mt-5 space-y-2.5">
                <Connection icon={Bot} name="Discord" detail="Identity, server membership, role sync, commands" status={integrationState.get("discord") ?? (workspace.discord_guild_id ? "connected" : "not connected")} href="/login" />
                <Connection icon={Gamepad2} name="Roblox" detail="Official OAuth identity and Open Cloud group access" status={integrationState.get("roblox") ?? (workspace.roblox_group_id ? "connected" : "not connected")} />
...
function WorkspaceOnboarding({ displayName, error }: { displayName: string; error?: string }) {
  return (
    <main className="onboarding-page">
      <header className="onboarding-nav">
        <Link href="/" className="flex items-center gap-2.5">
          <BrandMark />
          <span className="font-display font-extrabold">Nexora Rank</span>
        </Link>
      ...
      <section className="stage onboarding-stage">
        <div className="stage-field" aria-hidden="true" />
        <div className="stage-inner glass-strong onboarding-card">
          <span className="chip" data-tone="live">
            <Check className="size-3" aria-hidden="true" /> Discord identity verified
          </span>
          <h1>Build your first workspace.</h1>
          <p>
            Welcome, {displayName}. A workspace keeps one Roblox community, its Discord server,
            policies, API keys, and audit history together.
          </p>
          ...
          <form action={createWorkspace} className="mt-8 space-y-4">
            <label>
              ...
              <input name="name" required minLength={2} maxLength={64} placeholder="Nexora Community" />
            </label>
            <label>
              <span>Workspace URL</span>
              <div className="slug-input">
                <small>nexorarank.tech/w/</small>
                <input name="slug" required pattern="[a-z0-9][a-z0-9-]{1,46}[a-z0-9]" placeholder="nexora-community" />
              </div>
            </label>
            <button type="submit" className="pill pill-solid pill-lg w-full">
              Create secure workspace <ArrowRight className="size-4" aria-hidden="true" />
            </button>
          </form>

          <div className="onboarding-trust">
            <span><ShieldCheck /> Row-level access</span>
            <span><Link2 /> Official OAuth</span>
            <span><Webhook /> Audited actions</span>
          </div>
        </div>
      </section>
    </main>
...
function Connection({ icon: Icon, name, detail, status, href }: { icon: typeof Bot; name: string; detail: string; status: string; href?: string }) {
  const ready = status === "connected" || status === "ready";
  const content = (
    <>
