...
import { ArrowLeft, Bot, Check, Gamepad2, LoaderCircle, LockKeyhole, ShieldCheck, Sparkles } from "lucide-react";
    ...
    <main className="auth-page">
      ...
      <div className="auth-card">
        ...
        <div className="mt-8 grid grid-cols-3 gap-2">
          {["Official OAuth", "Scoped access", "Secure session"].map((item) => <div key={item} className="rounded-xl border border-white/[.06] bg-white/[.018] px-2 py-3 text-center"><Check className="mx-auto size-3.5 text-emerald-400" /><span className="mt-1.5 block text-[9px] text-white/32">{item}</span></div>)}
        </div>
        <p className="mt-7 text-center text-[10px] leading-5 text-white/22">Discord shares your identity, email, server list, and membership in a selected server. Nexora never asks for a Discord token or Roblox security cookie.</p>
