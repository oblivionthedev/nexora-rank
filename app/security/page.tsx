...
export default function SecurityPage() {
  ...
  return <LegalShell active="security"><LegalHeader eyebrow="Trust center" title="Security at Nexora Rank" summary="A practical security model for connecting Discord identity, Roblox community data, and sensitive staff operations." /><div className="security-control-grid">{controls.map(([Icon, title, text]) => <article key={title}><span><Icon className="size-5" /></span><h2>{title}</h2><p>{text}</p></article>)}</div><article className="legal-document"><section><h2>Data and secrets</h2><ul>{["Service credentials stay on the server and out of client bundles.", "Sensitive values are redacted from interface output and operational logs.", "Production tables use row-level security and workspace-scoped access rules.", "Transport encryption is required for browser, API, and provider traffic.", "Backups, retention, and incident procedures must be finalized before launch."].map((item) => <li key={item}>{item}</li>)}</ul></section><section><h2>Responsible disclosure</h2><p>Do not test against other users or workspaces, use social engineering, disrupt the service, or access more data than needed to demonstrate an issue. A dedicated security contact and safe-harbor policy will be published before the public beta. Until then, do not submit secrets through the preview.</p></section><section><h2>Current readiness</h2><p className="inline-flex items-center gap-2"><Check className="size-4 text-emerald-300" />Architecture and database migration prepared.</p><p>Dedicated Supabase project connection, OAuth provider setup, production domain, monitoring, recovery testing, and independent legal/security review remain launch gates.</p></section></article></LegalShell>;
}
