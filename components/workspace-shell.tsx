...
export function WorkspaceShell({ workspace, children }: { workspace: { public_id: string; name: string; operational_status: string; role: string }; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#030602] text-white lg:grid lg:grid-cols-[270px_minmax(0,1fr)]">
    <aside className="border-b border-white/8 bg-[#050806] p-5 lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r">
      <Link href="/" className="flex items-center gap-3 text-base font-bold"><BrandMark compact />Nexora Rank</Link>
      <div className="mt-7 rounded-[22px] border border-white/10 bg-white/[.035] p-4"><p className="text-[11px] font-bold uppercase tracking-[.15em] text-white/38">Workspace</p><p className="mt-2 truncate text-base font-bold">{workspace.name}</p><code className="mt-2 block text-xs text-[#0bda8e]">{workspace.public_id}</code></div>
      ...
      <div className="mt-6 hidden rounded-2xl border border-white/7 bg-black/20 p-4 text-xs leading-6 text-white/42 lg:block"><ShieldCheck className="mb-3 size-5 text-[#0bda8e]" />Signed in as <b className="text-white/70">{workspace.role}</b>. Workspace changes are recorded.</div>
    </aside>
...
export function PageHeading({ eyebrow, title, description, action }: { eyebrow: string; title: string; description: string; action?: React.ReactNode }) { return <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.17em] text-[#0bda8e]">{eyebrow}</p><h1 className="mt-3 text-4xl font-extrabold tracking-[-.04em] sm:text-5xl">{title}</h1><p className="mt-3 max-w-2xl text-base leading-7 text-white/52">{description}</p></div>{action}</div>; }
