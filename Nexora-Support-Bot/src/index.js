...
const support = createSupportService(client, config);
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    if (interaction.isButton() && interaction.customId.startsWith("ticket:member-close:")) {
      const channelId = interaction.customId.slice("ticket:member-close:".length);
      await interaction.update({ components: [] });
      await support.closeFromMember(interaction, channelId);
      return;
    }
    ...
    const messages = {
      ...
      support_team_role_missing:
        "The configured Nexora Support Team role is missing from this server.",
      ticket_member_required: "Only the member who opened this ticket can close it here.",
